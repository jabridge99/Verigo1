...
def get_report(
    ...
    r = _scoped(db, current_user).filter(ComplianceReport.report_id == report_id).first()
    if not r:
...
def update_report(
    ...
    for field, value in payload.model_dump(exclude_none=True).items():
        ...
        if field not in ("reviewed_by", "approved_by", "submitted_at", "acknowledged_at"):
...
def move_to_review(
    ...
    if r.status != ReportStatus.draft:
        raise HTTPException(400, f"Report must be in draft status (current: {r.status})")
...
def submit_report(
    ...
    if r.status != ReportStatus.approved:
        raise HTTPException(400, f"Report must be approved before submission (current: {r.status})")
...
def acknowledge_report(
    ...
    if r.status != ReportStatus.submitted:
        raise HTTPException(400, f"Report must be submitted before acknowledgement (current: {r.status})")
...
def create_ecdd(
    ...
    approved_kyc = db.query(KYCRecord).filter(
    ...
    if not approved_kyc and not (customer.risk_score or 0) >= 61:
        raise HTTPException(422, "ECDD requires an approved KYC record or risk score >= 61")
