...
from app.models.compliance_calendar import ComplianceCalendarItem, CalendarItemStatus
...
from app.models.report import IFTIReport, TTRReport, SMRReport, ReportStatus
...
def _open_alert_statuses():
    return [AlertStatus.generated, AlertStatus.assigned,
            AlertStatus.under_review, AlertStatus.escalated,
            AlertStatus.smr_candidate]

def _open_case_statuses():
    return [CaseStatus.open, CaseStatus.under_investigation,
            CaseStatus.additional_information, CaseStatus.escalated,
            CaseStatus.decision]
...
def global_dashboard(
    ...
    ifti_overdue = db.query(IFTIReport).filter(
        IFTIReport.org_id == org_id,
        IFTIReport.due_date < now,
        IFTIReport.status.in_(_pending_report_statuses()),
    ).count()
    ttr_overdue = db.query(TTRReport).filter(
        TTRReport.org_id == org_id,
    ...
    risk_score = min(
        (open_alerts * 2) + (critical_alerts * 5) + (open_cases * 3) +
        (smr_candidates * 8) + (ifti_overdue * 4) + (ttr_overdue * 4),
        100
    )
    org_risk_level = (
...
def remittance_dashboard(
    ...
    from app.services.risk_matrix_service import FATF_BLACKLIST, FATF_GREYLIST, SANCTIONED_COUNTRIES
    ...
    blacklist_txns = txn_q.filter(
        Transaction.transaction_date >= prev_30d,
        Transaction.destination_country.in_(list(FATF_BLACKLIST | SANCTIONED_COUNTRIES)),
    ).count()
...
def crypto_dashboard(
    ...
    from app.models.screening import CryptoWalletScreening, WalletRiskCategory, ScreeningStatus
    ...
    high_risk_wallets = wallet_q.filter(
        CryptoWalletScreening.risk_category.in_([
            WalletRiskCategory.high_risk, WalletRiskCategory.sanctioned,
        ]),
    ).count()
    ...
    crypto_alerts = db.query(TransactionAlert).filter(
        TransactionAlert.org_id == org_id,
        TransactionAlert.category.in_([
            AlertCategory.crypto_mixer, AlertCategory.darknet_exposure,
...
def legal_dashboard(
    ...
    from app.models.professional_assessment import (
        ProfessionalAssessment, AssessmentStatus, AssessmentRiskRating,
    ...
    pa_q = db.query(ProfessionalAssessment).filter(
        ...
        ProfessionalAssessment.professional_service_type.in_([
            ProfessionalServiceType.lawyer, ProfessionalServiceType.conveyancer,
        ]),
    )
    ...
    high_risk_matters = pa_q.filter(
        ProfessionalAssessment.overall_risk_rating.in_([
            AssessmentRiskRating.high, AssessmentRiskRating.critical,
        ])
    ).count()
    ...
    property_txns = db.query(Transaction).filter(
        Transaction.org_id == org_id,
        Transaction.transaction_type.in_(["real_estate_settlement"]),
        Transaction.transaction_date >= prev_30d,
    ).count()

    escalated = pa_q.filter(
        ProfessionalAssessment.is_escalated == True
    ).count()
...
def accountants_dashboard(
    ...
    from app.models.professional_assessment import (
        ProfessionalAssessment, AssessmentStatus, AssessmentRiskRating,
        ProfessionalServiceType, TaxRiskAssessment,
    )

    pa_q = db.query(ProfessionalAssessment).filter(
        ProfessionalAssessment.org_id == org_id,
        ProfessionalAssessment.professional_service_type.in_([
            ProfessionalServiceType.accountant, ProfessionalServiceType.tax_adviser,
    ...
    high_risk_clients = db.query(Customer).filter(
...
def real_estate_dashboard(
    ...
    pa_q = db.query(ProfessionalAssessment).filter(
        ProfessionalAssessment.org_id == org_id,
        ProfessionalAssessment.professional_service_type == ProfessionalServiceType.real_estate,
    )
    ...
    property_txns_30d = db.query(Transaction).filter(
        Transaction.org_id == org_id,
        Transaction.transaction_date >= prev_30d,
    ).count()

    # Foreign purchasers: cross-border transactions or customers with foreign nationality
    foreign_purchasers = db.query(Customer).filter(
        Customer.org_id == org_id,
...
def alert_trends(
    ...
    for i in range(weeks):
        ...
        count = db.query(TransactionAlert).filter(
            TransactionAlert.org_id == org_id,
...
def compliance_score(
    ...
    critical = db.query(TransactionAlert).filter(
        TransactionAlert.org_id == org_id,
        TransactionAlert.severity == AlertSeverity.critical,
        TransactionAlert.status.in_(_open_alert_statuses()),
    ).count()
    rpt_overdue = sum([
        db.query(IFTIReport).filter(
            IFTIReport.org_id == org_id, IFTIReport.due_date < now,
            IFTIReport.status.in_(_pending_report_statuses()),
        ).count(),
        db.query(TTRReport).filter(
            TTRReport.org_id == org_id, TTRReport.due_date < now,
            TTRReport.status.in_(_pending_report_statuses()),
        ).count(),
    ])
    cal_overdue = db.query(ComplianceCalendarItem).filter(
        ComplianceCalendarItem.org_id == org_id,
        ComplianceCalendarItem.due_date < now,
        ComplianceCalendarItem.status.in_([
            CalendarItemStatus.open, CalendarItemStatus.in_progress,
    ...
    edd_pending = db.query(Customer).filter(
        Customer.org_id == org_id,
        Customer.status == CustomerStatus.edd_required,
    ).count()
    escalated = db.query(Case).filter(
        Case.org_id == org_id, Case.status == CaseStatus.escalated,
    ).count()
