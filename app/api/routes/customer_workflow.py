...
from app.models.customer_workflow import (
    ...
    CustomerWorkflowEvent, WorkflowAction, WorkflowState, WORKFLOW_TRANSITIONS,
)
from app.models.user import User
from app.schemas.customer_workflow import (
    ApprovalRequest, EDDApprovalRequest, RFIRequest,
    RiskAssessmentRequest, RiskAssessmentResponse, RiskDimensionResponse,
    RiskProfileResponse, WorkflowActionRequest, WorkflowAssignRequest,
    WorkflowEventResponse, WorkflowResponse,
)
from app.services.customer_risk_engine import (
    assess_customer_risk, cdd_level_from_gateway, risk_level_from_score,
)

log = logging.getLogger("verigo.api.customer_workflow")
router = APIRouter(prefix="/customers/{customer_id}/workflow", tags=["Customer Workflow"])
...
def _get_customer_and_workflow(
    ...
    workflow = db.query(CustomerWorkflow).filter(
        CustomerWorkflow.customer_id == customer_id
    ).first()
    if not workflow:
...
def start_data_collection(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.start_collection, current_user, db, comments)
    workflow.started_at = datetime.now(timezone.utc)
    workflow.sla_due_date = workflow.sla_due_date or (date.today() + timedelta(days=SLA_DAYS["default"]))
    db.commit()
    db.refresh(workflow)
    return workflow
...
def submit_for_verification(
    ...
    if customer.customer_type.value in ("company", "trust", "partnership") and not customer.business_detail_id:
    ...
    _transition(workflow, WorkflowAction.submit_for_verification, current_user, db, payload.comments)
...
def record_verification_result(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    action = WorkflowAction.verification_passed if passed else WorkflowAction.verification_failed
...
def run_risk_assessment(
    ...
    profile = db.query(CustomerRiskProfile).filter(
        CustomerRiskProfile.workflow_id == workflow.id
    ).first()
    if profile:
        profile.version += 1
    else:
    ...
    cr, pr, gr, chr_, tr = (
        result.customer_risk, result.product_risk, result.geographic_risk,
        result.channel_risk, result.transaction_risk,
    )
    profile.customer_risk_score = cr.score
    profile.customer_risk_factors = cr.factors
    profile.is_pep = cr.flags.get("is_pep", False)
    profile.nationality_risk = "high" if cr.flags.get("blacklist_nationality") else "medium" if cr.score > 20 else "low"
    ...
    profile.countries_involved = list(set(filter(None, [
        customer.nationality, customer.country_of_residence, customer.country_of_birth,
    ...
    _transition(
        workflow, WorkflowAction.run_risk_assessment, current_user, db,
    ...
    route_action = WorkflowAction.route_edd if result.gateway_decision == "edd" else WorkflowAction.route_cdd
    _transition(
        workflow, route_action, current_user, db,
    ...
    if result.gateway_decision == "edd":
        ...
        db.add(CustomerRiskScoreHistory(
            ...
            cdd_level=new_cdd,
            scoring_factors={
                "customer": cr.factors, "product": pr.factors,
                "geographic": gr.factors, "channel": chr_.factors,
    ...
    state_after = WorkflowState.edd_required.value if result.gateway_decision == "edd" else WorkflowState.cdd_review.value
...
def start_edd_collection(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.start_edd_collection, current_user, db, comments)
    db.commit()
    db.refresh(workflow)
    return workflow
...
def submit_edd_for_approval(
    ...
    if chk and not (chk.edd_source_of_funds_verified and chk.edd_source_of_wealth_verified):
        raise HTTPException(
            422,
...
def approve_edd(
    ...
    if payload.approved:
        _transition(workflow, WorkflowAction.approve_edd, current_user, db, payload.notes)
        ...
        _transition(workflow, WorkflowAction.return_edd, current_user, db,
...
def submit_for_final_review(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.submit_for_approval, current_user, db, comments)
...
def request_more_information(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.request_information, current_user, db, payload.rfi_notes)
...
def escalate_to_edd(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.escalate_edd, current_user, db, comments,
...
def final_decision(
    ...
    if workflow.state != WorkflowState.pending_approval:
        raise HTTPException(422, f"Decision requires 'pending_approval' state; current: '{workflow.state.value}'")
    ...
    action = WorkflowAction.approve if payload.decision == "approve" else WorkflowAction.reject
...
def start_periodic_review(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.trigger_periodic_review, current_user, db, comments)
    db.commit()
    db.refresh(workflow)
    return workflow
...
def complete_periodic_review(
    ...
    customer, workflow = _get_customer_and_workflow(customer_id, org_id_for(current_user), db)
    _transition(workflow, WorkflowAction.complete_periodic_review, current_user, db, comments)
    db.commit()
    db.refresh(workflow)
    return workflow
