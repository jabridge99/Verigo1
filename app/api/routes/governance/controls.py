...
def _refresh_control_effectiveness(control: GovernanceControl, db: Session) -> None:
    ...
    findings = (
        ...
        .filter(ControlTestFinding.test_id == latest.id)
        .all()
    )
    ...
    if (
        rating == ControlEffectiveness.ineffective
        and control.status == ControlStatus.active
    ):
...
def create_test(
    ...
    if payload.passed_samples + payload.failed_samples > payload.sample_size:
        raise HTTPException(
            422, "passed_samples + failed_samples cannot exceed sample_size"
        )
...
def list_tests(
    ...
    return (
        db.query(ControlTest)
        .filter(ControlTest.control_id == control_id)
        .order_by(ControlTest.test_date.desc())
        .all()
    )
...
@router.post(
    "/{control_id}/tests/{test_id}/finalise", response_model=ControlTestResponse
)
def finalise_test(
    control_id: str,
    test_id: str,
    ...
    findings = (
        db.query(ControlTestFinding).filter(ControlTestFinding.test_id == test_id).all()
    )
...
@router.post(
    "/{control_id}/remediations", response_model=RemediationResponse, status_code=201
)
...
def update_remediation(
    ...
    if payload.status == RemediationStatus.completed:
        if not (payload.closure_notes or rem.closure_notes):
            raise HTTPException(
                422, "closure_notes required when completing remediation"
            )
...
def list_open_remediations(
    ...
    q = db.query(ControlRemediationAction).filter(
        ...
        ControlRemediationAction.status.in_(
