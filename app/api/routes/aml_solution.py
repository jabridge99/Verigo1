...
from app.api.deps import (
    get_current_user, org_id_for, require_compliance_or_above, require_mlro_or_above,
)
...
from app.schemas.aml_solution import (
    ...
    AMLServiceRequestUpdate, AMLServiceResponse, AMLSolutionResponse,
)
...
def _get_program(solution_id: str, org_id: str, db: Session) -> AMLProgram:
    prog = db.query(AMLProgram).filter(
        AMLProgram.solution_id == solution_id,
        AMLProgram.org_id == org_id,
    ).order_by(AMLProgram.created_at.desc()).first()
    if not prog:
...
def list_services(
    ...
    sol = _get_solution(org_id_for(current_user), db)
    return db.query(AMLService).filter(
        AMLService.solution_id == sol.id
    ).order_by(AMLService.created_at).all()
...
def update_service(
    ...
    svc = db.query(AMLService).filter(
