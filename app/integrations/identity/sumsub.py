...
class SumsubProvider(IdentityProvider):
    ...
    def __init__(self, app_token: str, secret_key: str, base_url: str):
        if not app_token or not secret_key:
            raise ProviderUnavailableError("sumsub", "SUMSUB_APP_TOKEN / SUMSUB_SECRET_KEY not configured")
    ...
    async def _request(self, method: str, path: str, json: dict | None = None) -> dict:
        ...
        if resp.status_code >= 400:
            raise ProviderRejectedError("sumsub", f"{resp.status_code}: {resp.text}", raw=resp.text)
    ...
    def parse_webhook_event(self, payload: dict) -> VerificationStatus:
        ...
        return self._status_from_review(applicant_id, external_user_id, payload.get("reviewResult", payload))

    def _status_from_review(self, applicant_id: str, external_user_id: str | None, review: dict) -> VerificationStatus:
        review_result = review.get("reviewResult", {}) if "reviewResult" in review else review
        answer = (review_result.get("reviewAnswer") if isinstance(review_result, dict) else None)
        if answer in _PASS:
            normalised = "pass"
        ...
        return VerificationStatus(
            ...
            rejection_labels=(review_result.get("rejectLabels", []) if isinstance(review_result, dict) else []),
            raw=review,
        )
