...
FATF_GREY_LIST  = {
    "AF", "AL", "BB", "BF", "CM", "CF", "CD", "HT", "JM", "JO",
    "ML", "MZ", "NG", "PK", "PA", "PH", "SN", "SO", "SS", "SY",
    "TZ", "TN", "UG", "AE", "VU", "VN", "YE",
}
...
HIGH_CORRUPTION_COUNTRIES = {
    "SO", "SS", "SY", "YE", "VE", "AF", "CF", "LY", "HT", "NI",
    "GN", "GW", "SD", "ER", "CG", "TD", "MM", "TJ", "TM", "UZ",
}
...
class RegulatoryIndicator:
    ...
    indicator_type: str        # "potential_ifti", "potential_ttr", "potential_smr", "potential_edd", etc.
    title: str
    rationale: str
    regulatory_basis: str      # e.g. "AML/CTF Act 2006 s.45"
    priority: str              # "urgent", "high", "medium", "low"
    industry_specific: bool = False
    auto_prefill_type: Optional[str] = None  # "ifti", "ttr", "smr" — enables draft report generation
...
def evaluate_transaction(
    ...
    country_origin = str(getattr(transaction, "country_origin", source_country) or "").upper()
    country_destination = str(getattr(transaction, "country_destination", destination_country) or "").upper()
    ...
    cross_border_methods = {
        "swift", "cross_border_transfer", "remittance",
        "bank_transfer", "crypto", "digital_wallet",
    }
    effective_cross_border = is_cross_border or payment_method in cross_border_methods or (
        bool(country_origin) and bool(country_destination) and country_origin != country_destination
    )

    # ── IFTI evaluation (AML/CTF Act s.45) ────────────────────────────────────
    if effective_cross_border and amount_aud >= IFTI_THRESHOLD_AUD:
        result.potential_ifti = True
        result.indicators.append(RegulatoryIndicator(
    ...
    if is_structuring:
        smr_reasons.append("Structuring indicators detected (transactions near reporting threshold)")
    if is_pep:
    ...
    if alert_score >= HIGH_ALERT_SCORE_FOR_SMR:
        smr_reasons.append(f"Composite alert score {alert_score:.0f}/100 exceeds SMR consideration threshold")
    ...
    if crypto_detail:
        ...
        sanctioned_pct = float(getattr(crypto_detail, "sanctioned_exposure_pct", 0) or 0)
        ...
        if darknet_pct >= DARKNET_EXPOSURE_ALERT_PCT:
            smr_reasons.append(f"Crypto wallet has {darknet_pct:.1f}% darknet market exposure")
        if sanctioned_pct >= SANCTIONED_WALLET_ALERT_PCT:
            smr_reasons.append(f"Crypto wallet has {sanctioned_pct:.1f}% sanctioned entity exposure")
    ...
    if customer_risk_level in ("high", "critical"):
        edd_reasons.append(f"High/critical customer risk rating requires EDD review")
    ...
    if crypto_detail:
        ...
        if src_risk >= 70:
            edd_reasons.append(f"Source wallet risk score {src_risk:.0f}/100 requires EDD")
    ...
    if amount_aud >= 50_000:
        sof_reasons.append(f"Transaction value AUD {amount_aud:,.0f} warrants SOF verification")
    if is_pep:
        sof_reasons.append("PEP transactions require Source of Funds and Source of Wealth verification")
    ...
    if behaviour_profile:
        ...
        dormant_reactivated = bool(getattr(behaviour_profile, "dormant_reactivated", False))
    ...
    result.industry_guidance = _industry_guidance(
        org_industry, transaction_type, payment_method, amount_aud,
        effective_cross_border, crypto_detail, is_cash,
    )
    ...
    result.risk_summary = {
        ...
        "fatf_black_list_exposure": any(c in FATF_BLACK_LIST for c in countries_to_check),
        "fatf_grey_list_exposure": any(c in FATF_GREY_LIST for c in countries_to_check),
        "sanctioned_country_exposure": any(c in SANCTIONED_COUNTRIES for c in countries_to_check),
        "industry": org_industry,
    }

    log.debug(
        "regulatory_eval txn=%s ifti=%s ttr=%s smr=%s edd=%s indicators=%d",
        transaction.id, result.potential_ifti, result.potential_ttr,
        result.potential_smr, result.potential_edd, len(result.indicators),
    )
    return result
...
def _industry_guidance(
    ...
    if "remittance" in ind or "money_service" in ind or "msb" in ind:
        if is_cross_border:
            guidance.append("Remittance: Verify beneficiary identity and account details (FATF R.16 travel rule)")
            guidance.append("Remittance: Consider IFTI obligation for international transfers ≥ AUD 10,000")
        if is_cash:
            guidance.append("Remittance: Cash-funded remittances are high-risk — verify Source of Funds")
        if amount_aud > 0:
            guidance.append("Remittance: Check beneficiary country against FATF grey/black list and sanctions")

    elif "crypto" in ind or "digital_currency" in ind or "vasp" in ind or "blockchain" in ind:
        if crypto_detail:
            ...
            if mixer > 0:
                guidance.append(f"Crypto: Mixer exposure detected ({mixer:.1f}%) — assess SMR obligation (FATF VA Guidance 2021)")
            if darknet > 0:
                guidance.append(f"Crypto: Darknet exposure detected ({darknet:.1f}%) — immediate MLRO escalation recommended")
            guidance.append("Crypto: Verify wallet type (custodial vs self-hosted) per FATF travel rule requirements")
        guidance.append("Crypto: Apply enhanced monitoring for unhosted/self-hosted wallet transactions")

    elif "psp" in ind or "payment_service" in ind or "payment_facilit" in ind:
        guidance.append("PSP: Monitor merchant settlement volumes for structuring patterns")
        if is_cross_border:
            guidance.append("PSP: Cross-border settlement — verify merchant legitimacy and counterparty jurisdiction")
        guidance.append("PSP: Review merchant category codes for high-risk business types")

    elif "legal" in ind or "law" in ind or "solicitor" in ind or "conveyancing" in ind:
        guidance.append("Legal/Trust: Verify trust account transaction has proper client authorisation")
        guidance.append("Legal/Trust: Identify ultimate beneficial owner for property and corporate transactions")
        if amount_aud >= 10_000:
            guidance.append("Legal/Trust: Consider Source of Funds for transactions ≥ AUD 10,000 (FATF R.22)")
        if is_cash:
            guidance.append("Legal/Trust: Cash received in trust account — immediate SOF verification required")

    elif "real_estate" in ind or "property" in ind:
        guidance.append("Real Estate: Identify all purchasers and Ultimate Beneficial Owners")
        if amount_aud >= 100_000:
            guidance.append("Real Estate: High-value property transaction — Source of Wealth verification required")
        guidance.append("Real Estate: Screen for foreign purchasers and foreign investment restrictions")
        if is_cash:
            guidance.append("Real Estate: Cash component in property transaction — high money laundering risk")

    elif "accounting" in ind or "auditor" in ind or "tax_agent" in ind:
        guidance.append("Accounting: Verify client SOF where handling significant funds")
        guidance.append("Accounting: Document rationale for unusual transactions in client file")

    elif "gambling" in ind or "gaming" in ind or "wagering" in ind:
        guidance.append("Gambling: Apply customer due diligence for winnings/payouts ≥ reporting threshold")
        guidance.append("Gambling: Monitor for source of gambling funds — structuring risk")

    # General guidance always included
    if amount_aud >= IFTI_THRESHOLD_AUD:
        guidance.append(f"All industries: Transactions ≥ AUD {IFTI_THRESHOLD_AUD:,.0f} must be reviewed for reporting obligations")
...
def _required_actions(result: RegulatoryDecisionResult) -> list[str]:
    ...
    if result.potential_ifti:
        actions.append("Review for IFTI obligation — MLRO assessment required before processing")
    if result.potential_ttr:
        actions.append("Review for TTR obligation — complete threshold transaction assessment")
    if result.potential_smr:
        actions.append("Escalate to MLRO for SMR consideration — do not tip off customer")
    if result.potential_edd:
        actions.append("Initiate Enhanced Due Diligence process before transaction approval")
    ...
    if result.potential_customer_review:
        actions.append("Schedule customer profile review — behaviour deviation from baseline")
    return actions
...
def prefill_ifti_data(transaction, customer, org) -> dict:
    ...
    return {
        ...
        "suggested_fields": {
            "direction": "outgoing" if str(getattr(transaction, "direction", "")) == "outgoing" else "incoming",
...
def prefill_smr_data(transaction, customer, org, indicators: list[RegulatoryIndicator]) -> dict:
    """
    Pre-populate SMR report fields from transaction and alert data.
    """
    smr_reasons = [
        ind.rationale for ind in indicators
        if ind.indicator_type == "potential_smr"
    ]
    return {
        "prefilled": True,
        "disclaimer": DISCLAIMER,
        "suggested_fields": {
            ...
            "customer_name": getattr(customer, "full_name", None),
            "customer_dob": str(getattr(customer, "date_of_birth", "") or ""),
            "suspicious_activity_summary": "\n".join(smr_reasons) if smr_reasons else "",
            "transaction_ref": getattr(transaction, "transaction_ref", None),
            "account_number": getattr(transaction, "source_account_number", None),
        },
        "requires_mlro_review": True,
