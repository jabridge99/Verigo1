...
from app.models.governance_controls import (
    ControlEffectiveness, FindingSeverity, RemediationStatus,
...
def calculate_training_metrics(
    ...
    totals = {
        "assigned": 0, "exempt": 0, "completed": 0,
        "in_progress": 0, "overdue": 0, "expired": 0,
        "expiring_30d": 0, "active_cert": 0,
    }
        ...
        if status == TrainingStatus.completed:
            ...
            if exp is not None:
                ...
                if 0 <= delta <= 30:
                    ...
                    dept_map[dept]["expiring_30d"] = dept_map[dept].get("expiring_30d", 0) + 1
                if delta > 30:
...
def calculate_policy_metrics(
    ...
    return PolicyMetrics(
        ...
        under_review=counts.get("internal_review", 0) + counts.get("compliance_review", 0),
...
def calculate_control_health(
    ...
    quarter_ago = date(td.year, td.month - 2 if td.month > 2 else td.month + 10,
                       td.day) if td.month > 2 else date(td.year - 1, td.month + 10, td.day)
    ...
    for c in controls:
        total_active += 1
        eff = c.effectiveness.value if hasattr(c.effectiveness, "value") else str(c.effectiveness)
        eff_counts[eff] = eff_counts.get(eff, 0) + 1
        if c.last_tested_date and c.last_tested_date >= quarter_ago:
            tested_q += 1

    total = max(total_active, 1)
    effective_count = eff_counts.get("effective", 0) + eff_counts.get("largely_effective", 0)
    ...
    critical_open = sum(
        ...
        if getattr(r, "finding_severity", None) in (FindingSeverity.critical,)
           and r.status not in (RemediationStatus.completed, RemediationStatus.risk_accepted)
    )
    overdue_count = sum(
        1 for r in open_rem
    ...
    remediation_current_pct = round(
        (open_count - overdue_count) / max(open_count, 1) * 100, 2
    ) if open_count else 100.0
...
def policy_register_report(policies: Sequence[Any]) -> List[Dict[str, Any]]:
    ...
    for p in policies:
        rows.append({
            ...
            "review_due":     p.review_due_date.isoformat() if p.review_due_date else None,
...
def control_register_report(controls: Sequence[Any]) -> List[Dict[str, Any]]:
    ...
    for c in controls:
        rows.append({
            ...
            "name":           c.name,
            "risk_area":      c.risk_area.value if hasattr(c.risk_area, "value") else c.risk_area,
            "control_type":   c.control_type.value if hasattr(c.control_type, "value") else c.control_type,
            "owner":          c.control_owner,
            "business_unit":  c.business_unit,
            "effectiveness":  c.effectiveness.value if hasattr(c.effectiveness, "value") else c.effectiveness,
            "last_tested":    c.last_tested_date.isoformat() if c.last_tested_date else None,
...
def training_register_report(records: Sequence[Any]) -> List[Dict[str, Any]]:
    ...
    for r in records:
        rows.append({
            ...
            "training_type":  r.course.training_type.value if r.course and hasattr(r.course.training_type, "value") else None,
