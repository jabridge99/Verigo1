...
def run_monitoring(db, txn, customer, industry_id=None):
    ...
    if amount_aud >= CTR_THRESHOLD:
        alerts.append(TransactionAlert(
            alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
            industry_id=industry_id, alert_type=AlertType.large_transaction,
    ...
    below_threshold = [t for t in txns_24h if 7_000 <= (t.amount_aud or t.amount) < CTR_THRESHOLD]
    if len(below_threshold) >= 3:
        total = sum(t.amount_aud or t.amount for t in below_threshold)
        alerts.append(TransactionAlert(
            alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
            industry_id=industry_id, alert_type=AlertType.structuring,
            severity=AlertSeverity.high,
    ...
    # 3. Cross-border / IFTI
    if txn.is_cross_border or (txn.counterparty_country and txn.counterparty_country.upper() not in ("AU", "AUS", "")):
        sev = AlertSeverity.high if (txn.counterparty_country or "").upper() in HIGH_RISK_COUNTRIES else AlertSeverity.medium
        alerts.append(TransactionAlert(
            alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
            industry_id=industry_id, alert_type=AlertType.cross_border, severity=sev,
    ...
    if cc and cc in HIGH_RISK_COUNTRIES:
        alerts.append(TransactionAlert(
            alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
            industry_id=industry_id, alert_type=AlertType.high_risk_country,
            severity=AlertSeverity.high,
    ...
    if customer.is_pep and amount_aud >= 5_000:
        alerts.append(TransactionAlert(
            alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
            industry_id=industry_id, alert_type=AlertType.pep_transaction,
            severity=AlertSeverity.high,
    ...
    for window_label, window_delta in VELOCITY_WINDOWS.items():
        ...
        if count >= count_thresh or total >= amount_thresh:
            alerts.append(TransactionAlert(
                alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
                industry_id=industry_id, alert_type=AlertType.velocity_breach,
    ...
    if txn.counterparty_name:
        ...
        if sanctions["match_found"]:
            alerts.append(TransactionAlert(
                alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
                industry_id=industry_id, alert_type=AlertType.sanctions_match,
    ...
    if industry_id:
        try:
            ...
            for rule_result in matched:
                alerts.append(TransactionAlert(
                    alert_id=_make_alert_id(), transaction_id=txn.id, customer_id=customer.id,
                    industry_id=industry_id, alert_type=AlertType.rule_triggered,
...
def get_alert_queue(db, industry_id=None, severity=None, status=None, alert_type=None, customer_id=None, skip=0, limit=100):
    ...
    return q.order_by(TransactionAlert.created_at.desc()).offset(skip).limit(limit).all()
...
def monitoring_stats(db, industry_id=None):
    ...
    return {
        "total_alerts": len(alerts), "open_alerts": open_count,
        "by_severity": by_severity, "by_type": by_type, "by_status": by_status,
        "total_transactions": total_txns, "flagged_transactions": flagged_txns,
    }
