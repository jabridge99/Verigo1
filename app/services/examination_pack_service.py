...
from app.models.examination_pack import ExaminationPack, ExaminationPackStatus, EXAMINATION_SECTIONS
...
def _section_aml_program(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.aml_solution import AMLSolution, SolutionStatus, AMLProgram, RiskAssessment
    ...
    if solution:
        ...
        risk_assessments = [{
            ...
            "assessed_at": r.assessed_at.isoformat() if getattr(r, "assessed_at", None) else None,
    ...
    return {
        ...
        "aml_solution_status": solution.status.value if solution and solution.status else None,
        "programs": programs,
...
def _section_customer_profile(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.customer import Customer, RiskLevel, CDDLevel, CustomerStatus

    customers = db.query(Customer).filter(Customer.org_id == org_id).all()
    period_customers = [
        c for c in customers
        if c.created_at and start <= c.created_at.date() <= end
    ]

    risk_dist = {"low": 0, "medium": 0, "high": 0, "critical": 0, "unassessed": 0}
    cdd_dist = {}
    for c in customers:
        lvl = (c.risk_level.value if c.risk_level and hasattr(c.risk_level, "value")
               else str(c.risk_level or "")).lower()
        if lvl in risk_dist:
            risk_dist[lvl] += 1
        else:
            risk_dist["unassessed"] += 1
        cdd = (c.cdd_level.value if c.cdd_level and hasattr(c.cdd_level, "value")
    ...
    return {
        ...
        "sanctions_match_count": sum(1 for c in customers if getattr(c, "is_sanctions_match", False)),
        "edd_customers": sum(1 for c in customers if cdd_dist.get("edd", 0)),
        "high_plus_risk_count": risk_dist["high"] + risk_dist["critical"],
    }


def _section_transaction_monitoring(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.monitoring import MonitoringRule, RuleExecution, TransactionAlert, AlertStatus, AlertSeverity, RuleStatus

    active_rules = db.query(MonitoringRule).filter_by(
        org_id=org_id, status=RuleStatus.active
    ).count()
...
def _section_smr_register(db: Session, org_id: str, start: date, end: date) -> dict:
    ...
    return {
        ...
        "pending_count": status_dist.get("draft", 0) + status_dist.get("pending_review", 0),
        "records": records,
    }
...
def _section_ifti_register(db: Session, org_id: str, start: date, end: date) -> dict:
    ...
    ifti_e = db.query(IFTIERecord).filter(
    ...
    return {
        ...
        "ifti_dra_records": [
            {"id": i.id, "report_ref": getattr(i, "report_ref", i.id),
             "status": i.status.value if hasattr(i.status, "value") else str(i.status),
             "created_at": i.created_at.date().isoformat() if i.created_at else None}
            for i in iftis
        ],
        "ifti_e_records": [
            {"id": i.id, "status": i.status.value if hasattr(i.status, "value") else str(i.status),
             "created_at": i.created_at.date().isoformat() if i.created_at else None}
            for i in ifti_e
        ],
    }
...
def _section_ttr_register(db: Session, org_id: str, start: date, end: date) -> dict:
    ...
    return {
        ...
        "status_distribution": status_dist,
        "records": [
            {"id": t.id, "report_ref": getattr(t, "report_ref", t.id),
...
def _section_training_records(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.governance_training import GovernanceTrainingRecord, TrainingStatus, TrainingCourse
    ...
    mandatory_courses = db.query(TrainingCourse).filter_by(
        org_id=org_id, is_mandatory=True, is_active=True
    ).all()
    ...
    # Assessment flags
    from app.models.training_trigger import AssessmentOutcomeFlag, AssessmentFlagStatus
    open_flags = db.query(AssessmentOutcomeFlag).filter_by(
        org_id=org_id, status=AssessmentFlagStatus.open
    ).count()
    ...
    return {
        ...
        "mandatory_courses": [
            {"id": c.id, "name": c.name, "training_type": c.training_type,
...
def _section_independent_reviews(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.independent_review import (
        IndependentReview, ReviewFinding, ReviewAction,
        ReviewStatus, FindingRisk, ActionStatus
    )
    ...
    for review in reviews:
        ...
        for f in findings:
            risk = f.risk_rating.value if hasattr(f.risk_rating, "value") else str(f.risk_rating or "low")
        ...
        actions = db.query(ReviewAction).filter_by(
            review_id=review.id
        ).filter(ReviewAction.status != ActionStatus.completed).count()
    ...
    return {
        ...
        "high_critical_findings": findings_summary["high"] + findings_summary["critical"],
        "reviews": [
            {
                "id": r.id,
                "review_ref": getattr(r, "review_ref", r.id),
                "status": r.status.value if hasattr(r.status, "value") else str(r.status),
                "review_type": r.review_type.value if hasattr(r.review_type, "value") else str(r.review_type),
            }
            for r in reviews
        ],
    }
...
def _section_policy_register(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.governance import Policy, PolicyLifecycleStatus, PolicyAttestation
    ...
    attestations_this_period = db.query(PolicyAttestation).filter(
        PolicyAttestation.org_id == org_id,
        PolicyAttestation.attested_at >= datetime.combine(start, datetime.min.time()),
        PolicyAttestation.attested_at <= datetime.combine(end, datetime.max.time()),
    ).count() if hasattr(PolicyAttestation, "org_id") else 0
        ...
        "policies": [
            {
                ...
                "title": p.title,
                "status": p.lifecycle_status.value if hasattr(p.lifecycle_status, "value") else str(p.lifecycle_status),
                "review_due_date": p.review_due_date.isoformat() if getattr(p, "review_due_date", None) else None,
...
def _section_control_testing(db: Session, org_id: str, start: date, end: date) -> dict:
    from app.models.governance_controls import GovernanceControl, ControlTest, ControlTestFinding

    controls = db.query(GovernanceControl).filter_by(org_id=org_id).all()
    tests_in_period = db.query(ControlTest).filter(
        ControlTest.control_id.in_([c.id for c in controls]),
        ControlTest.test_date >= start,
        ControlTest.test_date <= end,
    ).all() if controls else []

    findings = db.query(ControlTestFinding).filter(
        ControlTestFinding.test_id.in_([t.id for t in tests_in_period])
    ).all() if tests_in_period else []
...
def _section_notification_history(db: Session, org_id: str, start: date, end: date) -> dict:
    """
...
def export_html(pack: ExaminationPack) -> str:
    ...
    for section in (pack.sections or []):
        ...
        if "error" in data:
            section_html += f'<h2>{section.replace("_", " ").title()}</h2><p class="error">Section error: {data["error"]}</p>'
        else:
            rows = "".join(
    ...
    if pack.generation_errors:
        errors_html = "<h2>Generation Warnings</h2><ul>" + \
            "".join(f"<li>{e}</li>" for e in pack.generation_errors) + "</ul>"

    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>AUSTRAC Examination Pack — {pack.pack_ref}</title>
<style>
...
<div class="meta">
  <strong>Pack Reference:</strong> {pack.pack_ref} &nbsp;|&nbsp;
  <strong>Organisation:</strong> {org_info.get("name", pack.org_id)} &nbsp;|&nbsp;
  <strong>ABN:</strong> {org_info.get("abn", "—")} &nbsp;|&nbsp;
  <strong>AUSTRAC ID:</strong> {org_info.get("austrac_id", "—")}<br>
  ...
  <strong>Examiner:</strong> {pack.examiner_name or "—"} ({pack.examiner_agency})
</div>
...
<p style="font-size:11px;color:#888;">
  Generated by VeriGo Compliance Platform — {pack.pack_ref} — Version {pack.version}<br>
...
def export_csv_index(pack: ExaminationPack) -> str:
    ...
    for section in (pack.sections or []):
        ...
        for k, v in data.items():
            if not isinstance(v, (list, dict)):
                w.writerow([section.replace("_", " ").title(), k.replace("_", " ").title(), v])
    return buf.getvalue()
