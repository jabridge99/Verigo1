...
WORKFLOW_TRANSITIONS: dict[WorkflowState, list[WorkflowState]] = {
    ...
    WorkflowState.verification_pending:   [WorkflowState.verification_complete, WorkflowState.data_collection],
    ...
    WorkflowState.decision_gateway:       [WorkflowState.cdd_review, WorkflowState.edd_required],
    WorkflowState.edd_required:           [WorkflowState.edd_collection],
    WorkflowState.edd_collection:         [WorkflowState.edd_senior_approval],
    WorkflowState.edd_senior_approval:    [WorkflowState.cdd_review, WorkflowState.edd_collection],
    WorkflowState.cdd_review:             [WorkflowState.pending_approval, WorkflowState.on_hold, WorkflowState.edd_required],
    WorkflowState.pending_approval:       [WorkflowState.active, WorkflowState.rejected, WorkflowState.on_hold],
    WorkflowState.on_hold:                [WorkflowState.data_collection, WorkflowState.edd_collection,
                                           WorkflowState.cdd_review, WorkflowState.verification_pending],
    WorkflowState.active:                 [WorkflowState.periodic_review],
    WorkflowState.periodic_review:        [WorkflowState.active, WorkflowState.edd_required, WorkflowState.on_hold],
...
class CustomerWorkflow(Base):
    ...
    state = Column(Enum(WorkflowState), nullable=False, default=WorkflowState.draft, index=True)
    ...
    events = relationship("CustomerWorkflowEvent", back_populates="workflow",
                          cascade="all, delete-orphan", order_by="CustomerWorkflowEvent.occurred_at")
    risk_profile = relationship("CustomerRiskProfile", back_populates="workflow",
...
class CustomerWorkflowEvent(Base):
    ...
    workflow_id = Column(String, ForeignKey("customer_workflows.id", ondelete="CASCADE"), nullable=False, index=True)
...
class CustomerRiskProfile(Base):
    ...
    workflow_id = Column(String, ForeignKey("customer_workflows.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    ...
    customer_risk_factors = Column(JSON)            # breakdown: {"pep": 40, "nationality": 20, ...}
    ...
    expected_transaction_frequency = Column(String(50))     # daily | weekly | monthly | occasional
