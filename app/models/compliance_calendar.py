...
class ComplianceCalendarItem(Base):
    ...
    status = Column(Enum(CalendarItemStatus), default=CalendarItemStatus.scheduled,
    ...
    report_type = Column(String(20), nullable=True)  # ifti_incoming|ifti_outgoing|ttr|smr
    ...
    reminders = relationship("ComplianceReminder", back_populates="calendar_item",
...
class ComplianceReminder(Base):
    ...
    org_id = Column(String, ForeignKey("organisations.id"), nullable=False, index=True)
    calendar_item_id = Column(String, ForeignKey("compliance_calendar.id", ondelete="CASCADE"),
                               nullable=False, index=True)
