...
ALLOWED_TRANSITIONS: dict[PolicyLifecycleStatus, list[PolicyLifecycleStatus]] = {
    PolicyLifecycleStatus.draft:             [PolicyLifecycleStatus.internal_review,
                                              PolicyLifecycleStatus.archived],
    PolicyLifecycleStatus.internal_review:   [PolicyLifecycleStatus.compliance_review,
                                              PolicyLifecycleStatus.draft],
    PolicyLifecycleStatus.compliance_review: [PolicyLifecycleStatus.pending_approval,
                                              PolicyLifecycleStatus.internal_review],
    PolicyLifecycleStatus.pending_approval:  [PolicyLifecycleStatus.published,
                                              PolicyLifecycleStatus.compliance_review],
    PolicyLifecycleStatus.published:         [PolicyLifecycleStatus.periodic_review,
                                              PolicyLifecycleStatus.superseded,
                                              PolicyLifecycleStatus.archived],
    PolicyLifecycleStatus.periodic_review:   [PolicyLifecycleStatus.internal_review,
...
class Policy(Base):
    ...
    policy_category = Column(Enum(PolicyCategory), nullable=False,
                             default=PolicyCategory.operational)
    ...
    # ── Lifecycle ─────────────────────────────────────────────────────────────
    status = Column(Enum(PolicyLifecycleStatus),
                    default=PolicyLifecycleStatus.draft, nullable=False, index=True)
    ...
    # ── Cross-reference ───────────────────────────────────────────────────────
    superseded_by_id = Column(String, ForeignKey("governance_policies.id"), nullable=True)
    ...
    # ── Relationships ─────────────────────────────────────────────────────────
    versions      = relationship("PolicyVersion",
                                  back_populates="policy",
                                  cascade="all, delete-orphan",
                                  order_by="PolicyVersion.created_at")
    workflow_events = relationship("PolicyWorkflowEvent",
                                   back_populates="policy",
                                   cascade="all, delete-orphan",
                                   order_by="PolicyWorkflowEvent.occurred_at")
    attestations  = relationship("PolicyAttestation",
                                  back_populates="policy",
                                  cascade="all, delete-orphan")
    reminders     = relationship("PolicyReviewReminder",
                                  back_populates="policy",
                                  cascade="all, delete-orphan")
    superseded_by = relationship("Policy", foreign_keys=[superseded_by_id], uselist=False)
...
class PolicyVersion(Base):
    ...
    policy_id = Column(String, ForeignKey("governance_policies.id", ondelete="CASCADE"),
                       nullable=False, index=True)
...
class PolicyWorkflowEvent(Base):
    ...
    version_at_event = Column(String(20))  # e.g. "1.0" — version when this event occurred
...
class PolicyAttestation(Base):
    ...
    # ── What they attested ────────────────────────────────────────────────────
    attestation_type  = Column(Enum(AttestationType),
                                default=AttestationType.read_and_understood, nullable=False)
...
class PolicyReviewReminder(Base):
    ...
    review_due_date   = Column(Date, nullable=False)    # the policy review_due_date at scheduling time
