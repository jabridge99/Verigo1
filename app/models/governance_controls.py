...
class ControlEffectiveness(str, enum.Enum):
    ...
    partially_effective = "partially_effective" # 50–74% pass rate, or ≥1 moderate finding
...
class RemediationStatus(str, enum.Enum):
    ...
    completed    = "completed"
    overdue      = "overdue"
    risk_accepted = "risk_accepted"   # MLRO/Board accepted residual risk; closed without fix
...
class GovernanceControl(Base):
    ...
    linked_policy_id       = Column(String, ForeignKey("governance_policies.id"), nullable=True)
    ...
    frequency       = Column(Enum(ControlFrequency), nullable=False,
                             default=ControlFrequency.continuous)
    ...
    status           = Column(Enum(ControlStatus), default=ControlStatus.active,
                              nullable=False, index=True)
    effectiveness    = Column(Enum(ControlEffectiveness),
                              default=ControlEffectiveness.not_tested, nullable=False)
    ...
    # ── Relationships ─────────────────────────────────────────────────────────
    tests       = relationship("ControlTest",
                                back_populates="control",
                                cascade="all, delete-orphan",
                                order_by="ControlTest.test_date.desc()")
    evidence    = relationship("ControlEvidenceItem",
                                back_populates="control",
                                cascade="all, delete-orphan")
...
class ControlTest(Base):
    ...
    control_id  = Column(String, ForeignKey("governance_controls.id", ondelete="CASCADE"),
                          nullable=False, index=True)
    ...
    findings   = relationship("ControlTestFinding",
                               back_populates="test",
                               cascade="all, delete-orphan")
    remediations = relationship("ControlRemediationAction",
                                 back_populates="test",
                                 cascade="all, delete-orphan")
...
class ControlTestFinding(Base):
    ...
    test_id    = Column(String, ForeignKey("control_tests.id", ondelete="CASCADE"),
                         nullable=False, index=True)
    org_id     = Column(String, ForeignKey("organisations.id"), nullable=False)
...
class ControlRemediationAction(Base):
    ...
    # ── Status ────────────────────────────────────────────────────────────────
    status           = Column(Enum(RemediationStatus),
                               default=RemediationStatus.open, nullable=False, index=True)
