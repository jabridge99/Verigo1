...
class Case(Base):
    ...
    # ── Classification ────────────────────────────────────────────────────────
    case_type   = Column(Enum(CaseType), nullable=False, default=CaseType.internal_investigation)
    ...
    evidence        = relationship("CaseEvidence", back_populates="case",
                                   cascade="all, delete-orphan")
    alert_links     = relationship("CaseAlert", back_populates="case",
                                   cascade="all, delete-orphan")
...
class CaseAlert(Base):
    ...
    case_id         = Column(String, ForeignKey("cases.id", ondelete="CASCADE"),
                             nullable=False, index=True)
    alert_id        = Column(String, ForeignKey("transaction_alerts.id"), nullable=False, index=True)
...
class CaseNote(Base):
    ...
    note_type       = Column(Enum(NoteType), default=NoteType.investigation_note, nullable=False)
