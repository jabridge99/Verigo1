...
def upgrade() -> None:
    ...
    # these tables — see app/models/marketplace.py), they already exist.
    # create_all() never runs the seed insert below though, so that part
    # always still needs to run (guarded separately, by row presence).
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    tables = inspector.get_table_names()

    if "verification_providers" in tables and "verification_orders" in tables:
        _seed_if_empty(bind)
        return
    ...
    op.create_index(
        "ix_verification_providers_org_id", "verification_providers", ["org_id"]
    )
    op.create_index(
        "ix_verification_providers_check_type", "verification_providers", ["check_type"]
    )
    op.create_index(
        "ix_verification_providers_is_active", "verification_providers", ["is_active"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "provider_id",
            sa.String(),
            sa.ForeignKey("verification_providers.id"),
            nullable=False,
        ),
        ...
        sa.Column(
            "screening_record_id",
            sa.String(),
            sa.ForeignKey("screening_records.id"),
            nullable=True,
        ),
    ...
    op.create_index(
        "ix_verification_orders_provider_id", "verification_orders", ["provider_id"]
    )
    op.create_index(
        "ix_verification_orders_entity_type", "verification_orders", ["entity_type"]
    )
    op.create_index(
        "ix_verification_orders_entity_id", "verification_orders", ["entity_id"]
    )
    op.create_index("ix_verification_orders_status", "verification_orders", ["status"])
    op.create_index(
        "ix_verification_orders_usage_record_id",
        "verification_orders",
        ["usage_record_id"],
    )
    op.create_index(
        "ix_verification_orders_created_at", "verification_orders", ["created_at"]
    )
...
def _seed_if_empty(bind) -> None:
    existing_row = bind.execute(
        sa.text("SELECT 1 FROM verification_providers WHERE is_system IS TRUE LIMIT 1")
    ).first()
...
def _seed_default_providers() -> None:
    ...
    defaults = [
        ...
        (
            "Manual Sanctions Screening Review",
            "sanctions_screening",
            "manual",
            None,
            0.0,
        ...
        (
            "API Identity Verification (configure vendor)",
            "identity_verification",
            "api",
            None,
            0.0,
        ...
        (
            "API Sanctions Screening (configure vendor)",
            "sanctions_screening",
            "api",
            None,
            0.0,
        ...
        (
            "Hybrid Adverse Media Screening (configure vendor)",
            "adverse_media",
            "hybrid",
            None,
            0.0,
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_verification_orders_usage_record_id", table_name="verification_orders"
    )
    ...
    op.drop_index(
        "ix_verification_orders_entity_type", table_name="verification_orders"
    )
    op.drop_index(
        "ix_verification_orders_provider_id", table_name="verification_orders"
    )
    op.drop_index("ix_verification_orders_org_id", table_name="verification_orders")
    op.drop_table("verification_orders")

    op.drop_index(
        "ix_verification_providers_is_active", table_name="verification_providers"
    )
    op.drop_index(
        "ix_verification_providers_check_type", table_name="verification_providers"
    )
    op.drop_index(
        "ix_verification_providers_org_id", table_name="verification_providers"
    )
