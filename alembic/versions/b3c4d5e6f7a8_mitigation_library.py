...
def upgrade() -> None:
    ...
    # this table and these link columns), the schema already exists —
    # but create_all() never runs the seed insert below, so that part
    # always still needs to run (guarded separately, by row presence,
    # so it isn't duplicated on a database that already has this
    # migration's history behind it).
    bind = op.get_bind()
    inspector = sa.inspect(bind)

    if "mitigation_library_items" not in inspector.get_table_names():
        op.create_table(
        ...
        inspector = sa.inspect(bind)  # refresh — new table now exists

    rm_columns = {c["name"] for c in inspector.get_columns("risk_mitigations")}
    if "library_item_id" not in rm_columns:
        op.add_column(
            "risk_mitigations",
            sa.Column("library_item_id", sa.String(), nullable=True),
        )
    rm_indexes = {ix["name"] for ix in inspector.get_indexes("risk_mitigations")}
    if "ix_risk_mitigations_library_item_id" not in rm_indexes:
        op.create_index(
            "ix_risk_mitigations_library_item_id",
            "risk_mitigations",
            ["library_item_id"],
        )
    rm_fks = {fk["name"] for fk in inspector.get_foreign_keys("risk_mitigations")}
    if "fk_risk_mitigations_library_item_id" not in rm_fks:
    ...
    gc_columns = {c["name"] for c in inspector.get_columns("governance_controls")}
    if "library_item_id" not in gc_columns:
        op.add_column(
            "governance_controls",
            sa.Column("library_item_id", sa.String(), nullable=True),
        )
    gc_indexes = {ix["name"] for ix in inspector.get_indexes("governance_controls")}
    if "ix_governance_controls_library_item_id" not in gc_indexes:
        op.create_index(
            "ix_governance_controls_library_item_id",
            "governance_controls",
            ["library_item_id"],
        )
    gc_fks = {fk["name"] for fk in inspector.get_foreign_keys("governance_controls")}
    if "fk_governance_controls_library_item_id" not in gc_fks:
        op.create_foreign_key(
    ...
    existing_row = bind.execute(
        sa.text(
            "SELECT 1 FROM mitigation_library_items WHERE is_system IS TRUE LIMIT 1"
        )
    ).first()
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_governance_controls_library_item_id", table_name="governance_controls"
    )
    ...
    op.drop_index(
        "ix_mitigation_library_items_org_id", table_name="mitigation_library_items"
    )
