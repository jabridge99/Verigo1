...
def upgrade() -> None:
    ...
    # (create_all() against the *current* model, which already includes
    # ECDDRecord/ecdd_records — see app/models/report.py), this table
    # already exists. On a database that predates that baseline, this
    # migration is what creates it.
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "ecdd_records" in inspector.get_table_names():
        return
    ...
    op.create_table(
        ...
        sa.Column(
            "customer_id", sa.String(), sa.ForeignKey("customers.id"), nullable=False
        ),
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "completed", name="ecddstatus"),
            nullable=False,
