...
- **Inline Pydantic schemas** instead of `app/schemas/` (the convention most route files follow). **22 of 28 route files done, 2026-09-16** — see PARKING_LOT.md's "C2 pass 3" and "C2 pass 4" for detail. Remaining 6 (`aml_program.py`, `customers.py`, `documents.py`, `governance/training.py`, `org_config.py`, `transactions.py`) each need a merge judgement against an already-existing `app/schemas/*.py` file of a related name, not just a new module.
