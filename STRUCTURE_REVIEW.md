...
- **Inline Pydantic schemas** instead of `app/schemas/` (the convention most route files follow). **In progress, 2026-09-16** — re-counted at the actual scope (28 route files define at least one inline `BaseModel`, not the originally-estimated 13; see PARKING_LOT.md's "C2 pass 3" for the corrected count and the first 5-file batch done). Moving out file-by-file, smallest/lowest-risk first, per the original proposal.
