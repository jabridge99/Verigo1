...
- **No central frontend API client — IN PROGRESS 2026-09-17, see PARKING_LOT.md's "C2 pass 11" through "C2 pass 18".** A survey before writing anything found a partial client already existed (`lib/auth.ts`'s `apiFetch()`, already used at 214 of the ~250 real call sites) — the actual gap was 44 files each redeclaring the same base-URL constant, zero typed response interfaces, and 30 silently-swallowed fetch errors, not a from-scratch build. Scaffolded the shared layer at `web/lib/api/client.ts` (one `API_BASE`, typed `apiGet`/`apiPost`/`apiPatch`/`apiPut`/`apiDelete`) and migrated eight pilot resources onto it end-to-end: `customers` (5 files — a real bug the typing immediately surfaced, a page assigning an untyped API response into state typed for a different shape, which crashed on render the moment a real fetch succeeded), `analytics` (4 files — every `by_status`/`by_risk` dict is enum-keyed at runtime, not fixed, so `Record<string, number>` rather than a narrower literal was confirmed necessary against the live backend), `billing` (1 file, 12 of its 17 API call sites — the other 5 hit a genuinely different backend resource, `storage.py`, sharing the same page file, and were deliberately left alone at the time; a related `null`-vs-`undefined` type gap between the backend's `Optional[X] = None` fields and the page's pre-existing looser local types was fixed the same principled way), `organisations` (1 file, `lib/signup.ts`, 12 of its 15 `/api/v1/*` call sites — the other 3 hit `auth.py` and the separate `aml_program.py` resource despite similarly-named nested routes, both deliberately left alone; the new `lib/api/organisations.ts` module's functions/types are re-exported from `lib/signup.ts` rather than just moved, since three existing pages still import them by name from there), `storage` (the 6 call sites left out of the `billing` pilot, same page file — after this pass the page has no raw `apiFetch`/local `API` constant left at all; needed a new `apiPut` on the shared client, the first pilot whose mutating endpoints weren't `POST`/`PATCH`; also surfaced a second instance of the `customers`-pilot bug class, both storage panels spreading an entire untyped API response — including boolean fields — into a `Record<string,string>` form state, fixed with a field-filtering helper rather than a type-level workaround), and all three governance sub-resources — `governance/training` (1 file, all 11 call sites; preserved the page's original per-call demo-data fallback semantics by switching to `Promise.allSettled` rather than collapsing to one try/catch; the certificate-export link stayed a plain `<a href>` via a URL-builder function rather than being forced through a JSON-parsing fetch it doesn't need), `governance/policies` (1 file, all 5 call sites; a display-bug find in the page's own pre-existing type: its hand-written `PolicyType` union was missing `independent_review_policy`, a real backend enum value that every fresh org auto-seeds a document for, which would have silently rendered `undefined` in that row's type column), and `governance/controls` (1 file, all 4 call sites, closing out the governance set; a third instance of the `customers`-pilot bug class, this time in a demo-data fallback path synthesizing an incomplete test object that `any[]` had been hiding). `lib/pricing.ts`'s public, server-side/ISR-cached plan-price fetch was deliberately excluded from the `billing` migration — a different execution context from the client-only auth wrapper. Remaining: 29 files/~137 call sites, one resource at a time (a handful of pages per pass, tested each time — not a single big-bang change), plus the 30 silent-`catch{}` sites as a related but separate cleanup.
