...
- **Oversized backend route files** — `customers.py` (2377 lines), `reports.py` (1497), `governance/training.py` (1278), `screening.py` (1570) still need this; `risk_assessment.py` (1213 lines) — **RESOLVED 2026-09-16**, see PARKING_LOT.md's "C2 pass 6" — is now a package (`framework.py`/`assessments.py`/`library.py` under `risk_assessment/`), split along its real domain boundaries rather than by line count, with two real transcription bugs (a dropped log line, a widened permission check) caught by an AST-diff verification step before committing. Proposal for the rest: same treatment — split into sub-routers per sub-domain (e.g. `customers/kyc.py`, `customers/beneficial_owners.py`, `customers/screening.py` under a `customers/` package), one file at a time, each verified the same way.
