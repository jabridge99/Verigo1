...
- **Oversized backend route files** — `customers.py` (2377 lines), `screening.py` (1570) still need this. `risk_assessment.py` (1213 lines), `governance/training.py` (1278 lines) and `reports.py` (1497 lines) — **RESOLVED 2026-09-16**, see PARKING_LOT.md's "C2 pass 6"/"C2 pass 7"/"C2 pass 8" — are now packages, split along real domain boundaries rather than by line count, each verified via an AST-diff script comparing every function's exact source and every endpoint's permission dependency against the original (this caught two real transcription bugs in the first split — a dropped log line, a widened permission check — found zero in the second, and found a genuine pre-existing production bug in the third — an unreachable `/filing-register/export-csv` endpoint shadowed by `/filing-register/{entry_id}`, fixed as part of the split — confirming the check is cheap and worth keeping standard). Proposal for the rest: same treatment — split into sub-routers per sub-domain (e.g. `customers/kyc.py`, `customers/beneficial_owners.py`, `customers/screening.py` under a `customers/` package), one file at a time, each verified the same way.
