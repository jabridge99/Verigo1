...
// Full class strings written explicitly so Tailwind includes them at build time.
const ACTIVE = {
    ...
    <div className="min-h-screen flex bg-slate-50">
      ...
      >
        ...
        >
          ...
          <div>
            <div className="font-bold text-white text-[15px] tracking-tight leading-none">CirclLoop</div>
            <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 mt-0.5">{title}</div>
          </div>
        </Link>
        ...
        <nav className="flex-1 px-3 py-3 overflow-y-auto">
          {navItems.map((item, i) => {
            if (item.divider) {
              return (
                <div key={`div-${i}`} className="pt-5 pb-1.5 px-2 first:pt-2">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-slate-100 h-16 flex items-center justify-between px-4 sm:px-6 flex-shrink-0">
          <button className="lg:hidden p-2 -ml-1 text-slate-500" onClick={() => setOpen(v => !v)}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5 pl-3 border-l border-slate-100">
              ...
              <div className="hidden sm:block">
                ...
                {userRole && <div className="text-[11px] text-slate-400">{userRole}</div>}
              </div>
            </div>
          </div>
        </header>
