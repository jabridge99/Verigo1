...
def test_generate_risk_assessment(client, db):
    ...
    client.patch(f"/api/v1/organisations/{org['org_id']}", json={"risk_profile": "high"}, headers=headers)
    ...
    res = client.get(f"/api/v1/organisations/{org['org_id']}/risk-assessment", headers=headers)
...
def test_accountability_ack_writes_audit_log_and_blocks_false(client, db):
    ...
    entry = (
        db.query(AuditLog)
        .filter_by(action="aml_accountability_acknowledged", entity_id=org["org_id"])
        .first()
    )
