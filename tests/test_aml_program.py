...
def test_non_member_cannot_generate_program(client, db):
    ...
    client.patch(f"/api/v1/organisations/{org['org_id']}", json={"risk_profile": "low"}, headers=_auth(owner))

    res = client.post(f"/api/v1/organisations/{org['org_id']}/aml-program/generate", headers=_auth(other))
