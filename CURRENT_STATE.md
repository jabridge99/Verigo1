...
3. **A second, unrelated critical baseline bug was found and fully fixed during Stage 1:** a genuinely fresh PostgreSQL database could not run `alembic upgrade head` at all (12 model modules were never imported into `Base.metadata`), and 9 downstream migrations weren't idempotent against the fix. Both fixed and re-verified end-to-end against real PostgreSQL 16 — a fresh database now migrates cleanly to `head` and the app boots and serves a working API against it. See `DEVELOPMENT.md` §5.
